...
controls.enableDamping=true;
controls.target.set(0, 0, 0);  // point the camera to the center of your solar system
controls.dampingFactor = 0.05;
controls.rotateSpeed = 0.8;    // control rotation speed
controls.zoomSpeed = 1.0;      // control zoom speed
...
function createStars() {
  ...
  for (let i = 0; i < 1000; i++)
    verts.push(
      ...[...Array(3)].map(() => THREE.MathUtils.randFloatSpread(2000))
    );
...
function animate() {
  ...
  planetLabels.forEach(({ mesh, label }) => {
    ...
    }px,${(1 - (v.y * 0.5 + 0.5)) * window.innerHeight}px)`;
  });
  controls.update();
